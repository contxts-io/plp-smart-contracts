...
contract ERC721LendingPoolETH20_01 is
    ...
    PausableUpgradeable
{
    ...
    function setSupportedCurrency(address supportedCurrency) public onlyOwner {
    ...
    function borrow(
        ...
        require(
            borrowedAmount <= (valuation * maxLTVBPS) / 10_000,
            "Can't borrow more than max LTV"
        );
        require(borrowedAmount < IERC20(_supportedCurrency).balanceOf(address(this)), "not enough money");
        
        IERC20(_supportedCurrency).transfer(msg.sender, borrowedAmount);
    ...
    function repay(uint256 nftID, uint256 repayAmount) public whenNotPaused {
        require(nftHasLoan(nftID), "NFT does not have active loan");
        require(
